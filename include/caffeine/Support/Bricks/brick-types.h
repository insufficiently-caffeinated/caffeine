...
#include <type_traits>
    ...
    __has_include(<variant>) && __has_include(<optional>) // C++17
...
namespace types {
  ...
  struct NotPreferred {
    CONSTEXPR NotPreferred(Preferred) {}
  };
  ...
  namespace detail {
    ...
    template <typename T>
    constexpr auto _has_as_tuple(NotPreferred) {
      return false;
    }

    template <typename T>
    constexpr bool has_as_tuple() {
      return _has_as_tuple<T>(Preferred());
    }
  } // namespace detail
  ...
  using HasTrait =
      typename std::enable_if<std::is_base_of<Trait, T>::value, Ret>::type;
  ...
  struct Maybe : Comparable {
    ...
    T fromMaybe(T x) const {
      return isJust() ? value() : x;
    }

    explicit operator bool() const {
      return isJust() && bool(value());
    }
  ...
  struct StrongEnumFlags {
    ...
    explicit constexpr StrongEnumFlags(UnderlyingType st) noexcept
        : store(st) {}
    ...
    constexpr This operator~() const noexcept {
      return This(~store);
    }
  ...
  template <typename T>
  brq::parse_result from_string(std::string_view s,
                                brick::types::StrongEnumFlags<T>& f) {
    T val;
  ...
  namespace _impl {
    ...
    struct In<Needle, T, Ts...>
        : std::integral_constant<bool, std::is_same<Needle, T>::value ||
                                           In<Needle, Ts...>::value> {};
    ...
    struct NoneConvertible {
      using T = To;
    };

    template <typename From, typename To, typename T, typename... Ts>
    struct NoneConvertible<From, To, T, Ts...>
        : std::conditional<std::is_convertible<From, T>::value, Unit,
                           NoneConvertible<From, To, Ts...>>::type {};

    static_assert(std::is_convertible<Witness<int>, Witness<int>>::value,
                  "is_convertible");

    template <typename Needle, typename T, typename... Ts>
    struct _OneConversion<Needle, T, Ts...>
        : std::conditional<std::is_convertible<Needle, T>::value,
                           NoneConvertible<Needle, T, Ts...>,
                           _OneConversion<Needle, Ts...>>::type {};

    template <typename Needle, typename... Ts>
    struct OneConversion
        : std::conditional<In<Needle, Ts...>::value, Witness<Needle>,
                           _OneConversion<Needle, Ts...>>::type {};

    static_assert(std::is_same<OneConversion<int, int>::T, int>::value,
                  "OneConversion");
    static_assert(std::is_same<OneConversion<long, int>::T, int>::value,
                  "OneConversion");
    static_assert(
        std::is_same<OneConversion<long, std::string, int>::T, int>::value,
        "OneConversion");
    static_assert(
        std::is_same<OneConversion<long, int, long, int>::T, long>::value,
        "OneConversion");

    template <typename T, typename... Ts>
    struct AllDistinct<T, Ts...>
        : std::integral_constant<bool, !In<T, Ts...>::value &&
                                           AllDistinct<Ts...>::value> {};
    ...
    struct _ApplyResult<F, T, Fallback,
                        decltype(std::declval<F>()(std::declval<T&>()), true)> {
      using Parameter = T;
    ...
    struct ApplyResult<F, T, Ts...>
        : _ApplyResult<F, T, ApplyResult<F, Ts...>> {};

    template <typename F>
    struct ApplyResult<F> {};
  ...
  namespace _impl {
    ...
    struct Union : Comparable {
      ...
      template <typename T,
                typename U = typename _impl::OneConversion<T, Types...>::T>
      ...
      explicit operator bool() const {
        auto rv = const_cast<Union*>(this)->apply(
      ...
      // invoke `f` on the stored value if the type currently stored in the
      // union can be legally passed to that function as an argument
      template <typename F>
    ...
    private:
      ...
      template <unsigned char>
      void _copyAssignSame(const Union&) {
        UNREACHABLE("invalid _copyAssignSame");
      }
      ...
      template <unsigned char>
      void _destruct(unsigned char) {
        UNREACHABLE("invalid _destruct");
      }
      ...
      template <unsigned char>
      void _moveAssignSame(Union&&) {
        UNREACHABLE("invalid _moveAssignSame");
      }
      ...
      template <typename F>
      Applied<F> _apply(Preferred, F) {
        return Applied<F>::Nothing();
      }

      template <typename F, typename T>
      auto fixvoid(F f) ->
          typename std::enable_if<std::is_void<typename Applied<F>::T>::value,
                                  Applied<F>>::type {
        f(get<T>());
      ...
      auto _apply(Preferred, F f)
          -> Maybe<typename _impl::_ApplyResult<F, T, Unit>::Result> {
        if (!is<T>())
      ...
      bool _compare2(const Union&) const {
        UNREACHABLE("invalid discriminator");
      }

      template <template <typename> class Compare, int d, typename T,
                typename... Ts>
      ...
      auto _convert2(Preferred) const
          -> decltype(static_cast<Target>(this->unsafeGet<T>())) {
        if (_discriminator == d)
      ...
      void _swapSame(Union&) {
        UNREACHABLE("Invalid _swapSame");
      }
      ...
      template <unsigned char i>
      void _swapDifferent(Union&) {
        UNREACHABLE("Invalid _swapDifferent");
      }
      ...
      template <unsigned char local, typename Local, unsigned char i>
      void _swapDifferent2(Union&) {
        UNREACHABLE("Invalid _swapDifferent2");
      }

      template <unsigned char l, unsigned char r, typename L, typename R>
      auto _doSwapDifferent(Union& other) ->
          typename std::enable_if<l != 0 && r != 0>::type {
  ...
  template <typename... Types>
  using Union =
      _impl::Union<(std::is_copy_constructible<Types>::value && ...), Types...>;
  ...
  struct Either : Union<Left, Right> {
    ...
    bool isLeft() const {
      return this->template is<Left>();
    }
    bool isRight() const {
      return this->template is<Right>();
    }

    Left& left() {
      return this->template get<Left>();
    }
    Right& right() {
      return this->template get<Right>();
    }
  ...
  template <typename Fn, typename R = typename std::result_of<Fn()>::type>
  Lazy<Fn, R> lazy(Fn&& fn) {
    return Lazy<Fn, R>(std::forward<Fn>(fn));
  }
  ...
  struct _OneUnion
      : std::enable_if<(IsUnion<A>::value || IsUnion<B>::value) &&
                           !(IsUnion<A>::value && IsUnion<B>::value),
                       bool> {};

  template <typename A, typename B>
  auto operator==(const A& a, const B& b) ->
      typename std::enable_if<IsUnion<A>::value && !IsUnion<B>::value,
                              bool>::type {
  ...
  struct Overloaded<F, Fs...> : F, Overloaded<Fs...> {
    ...
    Overloaded(Ff&& f, Ffs&&... fs)
        : F(std::forward<Ff>(f)), Overloaded<Fs...>(std::forward<Ffs>(fs)...) {}
    using F::operator();
  ...
  struct Overloaded<F> : F {
    ...
    Overloaded(Ff&& f) : F(std::forward<Ff>(f)) {}
    using F::operator();
  };
  ...
  auto overloaded(F&& f, Fs&&... fs) {
    using O =
        Overloaded<std::remove_reference_t<F>, std::remove_reference_t<Fs>...>;
   ...
   * callables.
   * That is, visit_alternatives( v, fs... ) calls the function from fs which
   * best matches the type stored in the variant v.
   */
  ...
  namespace _impl {
    ...
    template <typename T>
    using OptV = std::conditional_t<std::is_same<T, void>::value, bool,
                                    std::optional<T>>;

    template <typename, typename>
    struct AppliedHelper;
    ...
    template <typename V>
    bool has_value(const V& v) {
      return v.has_value();
    }

    inline bool has_value(bool v) {
      return v;
    }
   ...
   * matched.
   * If the return value of the first callable is void, match returns bool
   * (false means not matched). Compared to visit_alternatives, this function is
   * slower as it goes through the callable one by one from the first to the
   * last.
   */
  ...
  auto match(Variant&& v, F&& f, Fs&&... fs) {
    ...
    return _impl::match<R>(std::forward<Variant>(v), std::forward<F>(f),
                           std::forward<Fs>(fs)...);
  }
...
template <typename Self, typename = typename std::enable_if<
                             brick::types::is_enum_class<Self>::value>::type>
constexpr brick::types::StrongEnumFlags<Self> operator|(Self a,
                                                        Self b) noexcept {
...
namespace t_types {
  ...
  struct IntegerEq : Eq {
    ...
    bool operator==(const IntegerEq& o) const {
      return val == o.val;
    }
  };
  ...
  struct IntegerEqOrd : Ord {
    ...
    bool operator==(const IntegerEqOrd& o) const {
      return val == o.val;
    }
    bool operator<(const IntegerEqOrd& o) const {
      return val < o.val;
  ...
  struct IntegerOrd : Ord {
    ...
    bool operator<(const IntegerOrd& o) const {
      return val < o.val;
    }
  };
  ...
  struct OverloadedTest {
    ...
    TEST(cast) {
      auto f1 =
          overloaded([](long) { return 0; }, [](std::string) { return 1; });
      ASSERT_EQ(f1(0), 0);
      ASSERT_EQ(f1("ahoj"), 1);

      auto f2 = overloaded([](long) { return 0; }, [](auto&&) { return 1; });
      ASSERT_EQ(f2(0), 1);
  ...
  struct VisitAlternativesTest {
    ...
    TEST(templated) {
      ...
      auto x = visit_alternatives(
          v, [](int) { return 1; }, [](std::string&) { return 2; },
          [](auto&&) { return 3; });
      ASSERT_EQ(x, 3);
  ...
  struct MatchTest {
    ...
    TEST(order) {
      ...
      auto x = match(
          v, [](std::string&) { return 1; }, [](long) { return 2; },
          [](int) { return 3; });
      ASSERT(x.has_value());
      ASSERT_EQ(x.value(), 2);
    }

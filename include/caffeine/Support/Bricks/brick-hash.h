...
namespace brq
{
    ...
    struct hash_state
    {
        ...
        uint64_t hash() const
        {
            ...
            return impl::mix64( a_ + c_, impl::prime_4 ) + impl::mix64( b_ + d_, impl::prime_5 );
        }
        ...
        template< typename T >
        std::enable_if_t< std::is_integral_v< T > && sizeof( T ) <= 8 > update_aligned( T v )
        {
    ...
    static inline hash64_t hash( const uint8_t *data, size_t len, hash64_t seed = 0 )
    {
        hash_state s( seed );

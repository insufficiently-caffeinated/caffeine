...
struct LLVMTransformer {
    ...
    constexpr decltype(auto) operand( size_t idx ) noexcept {
        if constexpr ( brick::types::is_detected_v< get_operand_t, std::remove_pointer_t< Value > > ) {
...
struct Annotation : brick::types::Eq {
    ...
    Annotation ns() const { return Annotation( _parts.begin(), _parts.end() - 1); }
        ...
        for ( const auto &op : arr->operands() ) {
            ...
            if ( auto val = cs.operand( 0 ).operand( 0 ).cast< Value >().freeze() ) {
                auto anno = cs.operand( 1 ).operand( 0 )
                           .cast< ::llvm::GlobalVariable >().operand( 0 )
...
template< typename Value, typename Yield >
void enumerateAnnosInNs( const std::string& ns, ::llvm::Module &m, Yield &&yield ) {
...
template< typename Value, typename Yield >
void enumerateForAnno( const std::string& anno, ::llvm::Module &m, Yield &&yield ) {
    enumerateForAnno< Value >( Annotation( anno ), m, std::forward< Yield >( yield ) );
}

template< typename Yield >
void enumerateFunctionsForAnno( const std::string& anno, ::llvm::Module &m, Yield &&yield ) {
...
template< typename Yield >
void enumerateFunctionAnnosInNs( const std::string& ns, ::llvm::Module &m, Yield &&yield ) {
    enumerateAnnosInNs< ::llvm::Function >( ns, m, std::forward< Yield >( yield ) );
}

template< typename Yield >
void enumerateFunctionsForAttribute( const std::string& attr, ::llvm::Module &m, Yield &&yield ) {
    for ( auto & fn : m )

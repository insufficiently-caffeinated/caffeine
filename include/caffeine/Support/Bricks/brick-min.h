...
struct is_printable<
    B, T, decltype(std::declval<B&>() << std::declval<const T&>(), void(0))>
    : std::true_type {};
template <typename B, typename T>
...
struct string_builder {
  ...
  string_builder() noexcept
      : _capacity(0), _offset(0), _hex(false), _oom(false) {}
  ~string_builder() noexcept {
    std::free(_buffer);
  }
  ...
  char* buffer_end() noexcept {
    return _buffer + _capacity - 1;
  }
  const char* buffer() const noexcept {
    return _buffer ? _buffer : "";
  }
  std::string_view data() const noexcept {
    return std::string_view(_buffer, _offset);
  }
  ...
  string_builder& hex(bool h = true) {
    _hex = h;
    return *this;
  }
  string_builder& dec() {
  ...
  string_builder& operator<<(std::u32string_view us) noexcept {
    while (!us.empty()) {
      ...
      } else // if (wc < 0x110000)
      {
        ...
        _buffer[_offset++] =
            static_cast<uint8_t>(0x80 | ((wc & 0x03F000) >> 12));
        _buffer[_offset++] =
            static_cast<uint8_t>(0x80 | ((wc & 0x000FC0) >> 6));
  ...
  template <typename P>
  auto operator<<(const P& p) noexcept
      -> decltype(*this << p.first << p.second) {
    return *this << "[" << p.first << ", " << p.second << "]";
  }

  template <typename F>
  auto operator<<(const F& f) noexcept -> decltype(f(*this), *this) {
    return f(*this), *this;
  }
...
void format_args(const char* sep, stream& acc, const A& a,
                 const Args&... as) noexcept {
  int mark = acc.size();
  if constexpr (nofail)
    format_nofail(acc, a);
  else
...
static inline split_view split(std::string_view p, char d,
...
static inline bool starts_with(std::string_view s,

...
struct PoolPointer : brick::types::Comparable
{
    ...
    Raw raw() const { return bitlevel::bitcast< Raw >( _rep ); }
    void raw( Raw r ) { bitlevel::bitcast( r, _rep ); }

    explicit operator bool() const { return slab(); }
    bool operator!() const { return !slab(); }
    bool operator<( PoolPointer o ) const { return intptr() < o.intptr(); }
    Raw intptr() const
    {
...
#pragma GCC diagnostic ignored "-Wold-style-cast"
        ...
        void valgrind_alloc( Pointer p, const char *mem, int size )
        {
            ...
            h[ p.chunk() ].handle =
                VALGRIND_CREATE_BLOCK( mem, size,
                                       brq::format( "blob ", p.slab(), p.chunk(), " @ ",
                                       static_cast< const void * >( mem ) ).buffer() );
        ...
        void valgrind_dealloc( Pointer p, const char *mem, int size )
        {
            ...
            vhandles[ p.slab() ][ p.chunk() ].handle =
                VALGRIND_CREATE_BLOCK( mem, size,
                                       brq::format( "blob ", p.slab(), p.chunk(), " @ ",
                                       static_cast< const void * >( mem ) ).buffer() );
...
struct RefPool : SlavePool< Master >
{
    ...
    static bool _no_cb( Pointer, T ) { return true; }
    ...
    void put( Pointer p, F cb = _no_cb )
    {
        ...
        if ( !rc ) /* TODO refactor */
        {
            ...
            const char *mem = block->data + p.chunk() * ( sz > 1 ? align( sz , 4 ) : sz );
...
struct RefCnt
{
    ...
    void get() { _pool.get( _ptr ); }
    void put() { _pool.put( _ptr ); }
    T count() { return _pool.count( _ptr ); }

    RefCnt() {}
    RefCnt( RefPool< Master, T > &s, Pointer p ) : _pool( s ), _ptr( p )    { get(); }
    RefCnt( const RefCnt &o ) : _pool( o._pool ), _ptr( o._ptr ) { get(); }
...
{
    return o << p.slab() << ":" << p.chunk() << " 0x" << std::hex << p.tag() << std::dec;
}

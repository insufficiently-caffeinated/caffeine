...
#define BRICK_ASSERT(t, l, ...)                                                \
  ::brq::assert_##t##_fn(BRICK_LOCATION(l), __VA_ARGS__)
...
#define BRICK_ASSERT_BIN(op, inv, x, ...)                                      \
  BRICK_ASSERT(                                                                \
      bin, #x " " #op " " BRICK_SHARP_FIRST(__VA_ARGS__, ignored),             \
      [](const auto& a, const auto& b) { return a op b; }, #op, #inv, x,       \
      __VA_ARGS__)

#define ASSERT(...)                                                            \
  BRICK_ASSERT(bool, BRICK_SHARP_FIRST(__VA_ARGS__, ignored), __VA_ARGS__)
...
template <typename... T>
__attribute__((noreturn)) void assert_die_fn(const trace_location& l,
                                             const T&... args) {
...
void assert_bin_fn(const trace_location& l, Op op, std::string_view op_str,
                   std::string_view inv, const A& a, const B& b,
                   const T&... args) {
  bool pass;
  ...
  trace_fn<true>(4, l, "assert:", l._statement, "[", a, op_str, b, "]",
                 args...);
}

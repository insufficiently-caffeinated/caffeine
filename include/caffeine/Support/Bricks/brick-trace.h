...
namespace brq
{
    ...
    template< typename T >
    [[gnu::destructor]] void singleton_destroy( T *x ) noexcept( std::is_nothrow_destructible_v< T > )
    {
    ...
    template< typename T >
    [[gnu::constructor]] T &singleton() noexcept( std::is_nothrow_default_constructible_v< T > )
    {
    ...
    struct trace
    {
        ...
        static std::string_view level( int i ) noexcept { return levels[ i ]; }
        ...
        void add_rule( std::string_view suffix, int level, int line = 0 ) noexcept ( nothrow )
        {
    ...
    static inline struct trace &trace() noexcept { return singleton< struct trace >(); }

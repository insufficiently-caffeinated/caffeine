...
class OperationSimplifier
    : public ConstOpVisitor<OperationSimplifier<move_out>, OpRef> {
private:
  using BaseType = ConstOpVisitor<OperationSimplifier<move_out>, OpRef>;
...
#define TRY_CONST_FLOAT(expr)                                                  \
  ...
  OpRef visit(const Operation& op) {
    ...
    return ConstOpVisitor<OperationSimplifier<move_out>, OpRef>::visit(op);
#else

...
namespace {
  class CttzIntrinsic : public ExternalFunction {
  public:
    void call(llvm::Function* func, InterpreterContext& ctx,
      ...
      for (unsigned bit = 0; bit < bitwidth; bit++) {
        ...
        auto ext = ctx.createExtract(arg, width);
...
std::unique_ptr<ExternalFunction> Intrinsics::cttz() {
  return std::make_unique<CttzIntrinsic>();
}

...
struct refcount_ptr {
  ...
  refcount_ptr(const refcount_ptr<T, atomic>& o) noexcept
      : refcount_ptr(o._ptr) {}
  refcount_ptr(T* v) noexcept : _ptr(v) {
    _get();
  }
  template <typename D,
  ...
  refcount_ptr<T, atomic>&
  operator=(const refcount_ptr<T, o_atomic>& o) noexcept {
    if constexpr (o_atomic ==
                  atomic) /* atomic and !atomic can't alias each other */
      if (&o == this)
        return *this;
  ...
  refcount_ptr<T, atomic>&
  operator=(const refcount_ptr<T, atomic>& o) noexcept {
  ...
  bool compare_exchange_strong(refcount_ptr<T, false>& p,
                               refcount_ptr<T, false> n) {
    TRACE("cmpxchg ", ptr(), p.ptr(), n.ptr());
    std::atomic<uintptr_t>& self =
        *reinterpret_cast<std::atomic<uintptr_t>*>(&_ptr);
